...
void Shader::set_uniform_int(const char* name, int value) const
{
    glUniform1i(glGetUniformLocation(this->id, name), value);
}

void Shader::set_uniform_int_array(const char* name, int count, const int* value) const
{
...
void Shader::read_inclusion_from_file(char* source, char* inclusion_line) const
{
    ...
    std::strcat(filepath, inclusion_line + std::strlen(INCLUSION_DIRECTIVE) + 2);
