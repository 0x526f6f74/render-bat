...
Camera::Config::Config(int viewport_width, int viewport_height) : viewport_width(viewport_width), viewport_height(viewport_height)
{ }
...
OrthographicCamera::Config::Config(int viewport_width, int viewport_height, float zoom_level)
  : Camera::Config(viewport_width, viewport_height)
  , aspect_ratio(static_cast<float>(viewport_width) / static_cast<float>(viewport_height))
  , zoom_level(zoom_level)
{ }

OrthographicCamera::Config::Config(float aspect_ratio, float zoom_level) : aspect_ratio(aspect_ratio), zoom_level(zoom_level)
{ }
...
PerspectiveCamera::PerspectiveCamera(const Config& config)
  : Camera(glm::perspectiveFov(glm::radians(config.fov), static_cast<float>(config.viewport_width), static_cast<float>(config.viewport_height), 0.1f, 1000.0f))
  , config(config)
{ }
...
void PerspectiveCamera::zoom_in(float zoom_level)
{
    ...
    this->projection_matrix =
        glm::perspectiveFov(glm::radians(config.fov), static_cast<float>(config.viewport_width), static_cast<float>(config.viewport_height), 0.1f, 1000.0f);
    this->dirty_view_projection_matrix = true;
}
...
void CameraController::update(double dt, const KeyboardState& keyboard) const
{
